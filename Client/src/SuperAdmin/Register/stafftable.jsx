...
const StaffTable = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:8081/account/staff/data')
      .then(response => {
  ...
  const handlePasswordResetSubmit = () => {
    ...
    axios.post('http://localhost:8081/admin/reset-password/staff', {
  ...
  const confirmDelete = () => {
    axios.delete(`http://localhost:8081/account/data/staff/${deleteIdNumber}`)
      .then(response => {
  ...
  const exportToExcel = () => {
    const dataToExport = data.map((user, index) => ({
      No: index + 1,
    ...
    XLSX.writeFile(workbook, "StaffDataRecord.xlsx");
  };
  ...
  return (
    ...
    <div className="m-5 w-full sm:w-[90%] mb-10 mt-16 shadow-3xl mx-auto border bg-[#F6FBFD] rounded-md font-montserrat ">
      <div className="flex justify-between h-[70px] items-center p-5 bg-[#292A84]">
        <h1 className="font-bold text-xl sm:text-2xl text-white">User List</h1>
        <button 
            ...
            {loading ? (
              ...
              currentRecords.map((user, index) => (
                <tr key={index} className="text-center hover:bg-gray-200">
                  ...
                  <td className="hidden sm:table-cell px-4 py-2 border-b-2">{user.idNumber}</td>
                  <td className="hidden sm:table-cell px-4 py-2 border-b-2">{user.designation}</td>
                  <td className="px-4 py-2 border-b-2">{user.email}</td>
                  <td className="hidden md:table-cell px-4 py-2 border-b-2">{user.username}</td>
      ...
      <div className="flex justify-end mt-16 mb-12 items-center gap-1 pr-10">
        ...
        <span className="px-3 py-1 text-sm border-t border-b border-blue-600 text-white bg-blue-500">
          {currentPage}
        </span>
        <button
          onClick={nextPage}
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 font-montserrat">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md text-center">
            <h2 className="text-lg font-bold mb-4">
              Reset Password for {currentUser.firstname} {currentUser.lastname}
            </h2>
            <div className="mb-4">
              <input
                type="text"
