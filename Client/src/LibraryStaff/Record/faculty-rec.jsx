    ...
    const fetchRecords = async () => {
      try {
        ...
        if (response.data.status) {
          const facultyRecords = response.data.data.filter(
            (record) => record.designation.toLowerCase() === 'faculty'
          );
  ...
  const updateStatus = async (id) => {
    try {
      ...
      if (response.data.status) {
        ...
        setRecords(
          records.map((record) =>
            record.id === id ? { ...record, status: editStatus[id] } : record
          )
        );
      } else {
    ...
    <div className="bg-[#EFF6FC] min-h-screen overflow-y-auto">
      ...
      <div className="m-5 w-full sm:w-[95%] mt-16 shadow-3xl mx-auto border bg-[#F6FBFD] rounded-md font-montserrat">
        <div className="flex justify-between h-[70px] items-center p-5 bg-[#292A84]">
          <h1 className="font-bold text-xl sm:text-2xl text-white">Faculty Borrowing List</h1>
          <button
              ...
              {currentRecords.length > 0 ? (
                currentRecords.map((record, index) => (
                  <tr key={record.id} className="text-center hover:bg-gray-200">
                    ...
                    <td className="border-b-2 px-4 py-2 whitespace-nowrap">
                      {moment(record.pickup_date).format("MMM Do YYYY")}
                    </td>
                    <td className="border-b-2 px-4 py-2 whitespace-nowrap">
                      {moment(record.estimated_date).format("MMM Do YYYY")}
                    </td>
                    <td className="border-b-2 px-4 py-2 whitespace-nowrap">
                      <select
                        name="status"
                        id="status"
                        className={`py-1 px-1 min-w-[150px] rounded-lg text-md border-2 text-center ${getStatusBackgroundColor(
                      ...
                      >
                        {getFilteredOptions(editStatus[record.id] || record.status).map((item) => (
                          <option className="text-black bg-white" value={item} key={item}>
                            {item}
                          </option>
                        ))}
                      </select>
                    ...
                    <td className="border-b-2 px-4 py-2 whitespace-nowrap">
                      {record.status !== "Returned" && (
                        <button
                          className="border bg-[#003687] text-white rounded-md px-3 py-1 cursor-pointer"
                ...
                <tr>
                  <td colSpan="9" className="text-center py-4 border text-red-500">
                    No records found
                  </td>
                </tr>
              )}
