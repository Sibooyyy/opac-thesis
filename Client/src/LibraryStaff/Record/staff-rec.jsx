    ...
    const fetchRecords = async () => {
      try {
        ...
        if (response.data.status) {
          const staffRecords = response.data.data.filter(record => record.designation.toLowerCase() === 'staff');
          ...
          <table className="w-full table-auto font-montserrat text-sm sm:text-md cursor-pointer">
            <thead className="text-xs sm:text-sm md:text-md font-semibold h-[45px] text-gray-700">
              <tr className="border-b-2 border-gray-500">
                ...
                <th className="hidden sm:table-cell px-4 py-4">Pickup Date</th>
            ...
            <tbody>
              {currentRecords.length > 0 ? (
                currentRecords.map((record, index) => (
                  <tr key={record.id} className='text-center hover:bg-gray-200'>
                    ...
                    <td className='border-b-2 px-4 py-2 whitespace-nowrap'>
                      ...
                      >
                        {getFilteredOptions(editStatus[record.id] || record.status).map(option => (
                ...
                <tr>
                  <td colSpan="9" className='text-center py-4 border text-red-500'>No records found</td>
                </tr>
              )}
            </tbody>
          </table>
